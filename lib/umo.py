...
for day in range(len(array_day)):
    ...
    for classroom in filtered_elements:
        cursor.execute('''SELECT * FROM timetable WHERE classroom = %s AND day_of_the_week = %s''', (classroom,str(array_day[day])))
        results = cursor.fetchall()
        for rows in results:
            if rows[7] == "0" and rows[2] != ' ' and rows[4] != 'None':
            ...
            if rows[7] == "1Н" and rows[2] != ' ' and rows[4] != 'None':
                sheet.cell(row=classroom_row + int(rows[4])*2-1, column=classroom_column).value = rows[3]
            if rows[7] == "2Н" and rows[2] != ' ' and rows[4] != 'None':
                sheet.cell(row=classroom_row + int(rows[4])*2, column=classroom_column).value = rows[3]
